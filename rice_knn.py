...
def main_rice():
    ...
    plot_validation_curves(train_results=train_results, test_results=test_results, title="Rice - KNN - Validation: Accuracy vs K (# Neighbors)", x_label="K", y_label="Accuracy", save_name="rice_KNN_validation_1_K", parameter_range=parameter_range, is_log=False)
    ...
    plt.title('Rice - KNN - Validation: Accuracy vs Distance Metrics (p)', fontsize=10)
    plt.xlabel('p')
    plt.ylabel('Accuracy')
    plt.grid()
    plt.savefig('rice_KNN_validation_2_p.png')
    plt.show()
    ...
    print(f"Rice - KNN - Optimized Accuracy: {classifier_accuracy}")
    print(f"Rice - KNNOptimized Parameters: {str(knn_optimized.best_params_)} \n")
    print(f"Rice - KNN - Time taken {str(end - start)}")

    ca.write(f"Rice - KNN - Optimized Accuracy: {classifier_accuracy}\n")
    ttr.write(f"Rice - KNN - Time taken {str(end - start)}\n")
    bp.write(f"Rice - KNNOptimized Parameters: {str(knn_optimized.best_params_)} \n")
    ...
    plot_learning_curves(x_range=param_range, train_results=train_results, test_results=test_results, title="Rice - KNN - Accuracy vs Learning Rate (Optimized Params)", x_label="Training Examples (%)", y_label="Accuracy", save_name="rice_KNN_learning_1.png",parameter_range=param_range)

...
def main_rice():
    ...
    plot_validation_curves(train_results=train_results, test_results=test_results, title="Rice - SVM - Validation: Accuracy vs C (Linear Kernel)", x_label="C", y_label="Accuracy", save_name="rice_SVM_validation_1_C", parameter_range=parameter_range, is_log=True)
    ...
    plt.legend(['Train Results', 'Test Results'])
    plt.title('Rice - SVM - Validation: Accuracy vs Kernel Learners', fontsize=10)
    plt.xlabel('Kernel')
    plt.ylabel('Accuracy')
    plt.grid()
    plt.savefig('rice_SVM_validation_2_kernel.png')
    plt.show()
    ...
    ca.write(f"Rice - SVM Optimized Accuracy: {classifier_accuracy}\n")
    bp.write(f"Rice - SVM Optimized Parameters: {str(model_svm_optimized.best_params_)} \n")
    ttr.write(f"Rice - SVM Train Time: {str(end-start)} \n")

    print(f"Rice - SVM Train Time: {str(end-start)} \n")
    print(f"Rice - SVM Optimized Parameters: {str(model_svm_optimized.best_params_)} \n")
    print(f"Rice - SVM Optimized Accuracy: {str(classifier_accuracy)}")

    svm_optimized = svm.SVC(random_state=620, C=model_svm_optimized.best_params_['C'], kernel=model_svm_optimized.best_params_['kernel'])
    ...
    plot_learning_curves(x_range=param_range, train_results=train_results, test_results=test_results, title="Rice - SVM - Accuracy vs Learning Rate (Optimized Params)", x_label="Examples Trained (%)", y_label="Accuracy", save_name="rice_SVM_learning_1.png",parameter_range=param_range)
